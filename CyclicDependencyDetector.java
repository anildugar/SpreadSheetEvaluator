...
final class CyclicDependencyDetector
{
	...
	public CyclicDependencyDetector(IDataManager datamanager, IMathematicalExpressionManager mathematicalexpressionmanager)
	{
	...
	/*
	 * Checks whether the spreadsheet is cyclic dependent.
	...
	public boolean IsSpreadSheetCyclicDependent()
	{
		...
		try
		{
			for (String tableCell : dataManager.getAllSpreadSheetCells())
			{
				...
				//If the expression contains raw values only, for eg. (4 5 *), no need to check for dependency
				...
				{					
					...
					//There is a possibility that there maybe an invalid cell as part of the expression, hence the entire spreadsheet input is invalid.
					...
					if (depedencyExpression.GetDependencySymbols().contains(tableCell))
					{
						...
						bIsCyclicDependent = true;
						System.out.println("There is a cyclic Dependency between " + tableCell + " and " + depedencySymbol);
					}
				}
			}
