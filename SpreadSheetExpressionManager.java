...
{
	/*This holds a raw expression which is provided as part of the input. Per cell it will return a mathematical expression.
	* The data is stored in the ordered format (A1, A2, A3, B1, etc).
	*/
	private LinkedHashMap<String, MathematicalExpression> rawExpressionByCell = new LinkedHashMap<String, MathematicalExpression>();
	
	/* All the mathematical expressions are evaluated one by one and the below variable holds the result of per cell mathematical expressions.
	*  The order is not maintained, the one which gets evaluated first, is stored first.
	*/
	...
	/*
	 * Construct all the Mathematical expression in the spreadsheet. Those expressions which contain rawvalues only will all also be evaluated.
	 */
	...
	/*
	 * Evaluates all the mathematical expressions which are all stored in a collection.
	 */
	...
	/*
	 * The mathematical expression could of 2 types RawValues and Placeholder.
	 * The input of the calculated expressions which are of type RawValues are stored in a collection and passed as input for placeholder expression
	 * This function keeps evaluating those mathematical expression which are Placeholder types.
	 * Every mathematical expression which is of type Placeholder will try to update the cells within the expression.    
	 * Once the expression type becomes RawValues, the evaluation of that particular expression is completed.
	 * Once All the expression type becomes RawValues, the spreadsheet evaluation is complete.
	 */
	private void evaluateExpressions(HashMap<String, MathematicalExpression> expressions)
	{
		...
		for (Map.Entry<String, MathematicalExpression> expressionByCell : expressions.entrySet())
		{
			...
			MathematicalExpressionType expressionType = expressionByCell.getValue().UpdateExpression(spreadSheetEvaluatedExpressions);
